...
;(function ($) {
    ...
    $.fn.extend({
         ...
         */
        memberInput: function (options, initUserName, dataList, dataMap) {
            /**
             * 默认配置项
             */
            var defaultOptions = {
                // 数据源
                allMemberUrl: 'api/productView/users',
                // 是否允许输入邮件组
            ...
            // 所有候选人列表，从服务器动态提取
            var allMemberList = dataList, allMemberMap = dataMap;
            ...
            MemberInput.prototype.inputFinished = function (dom, shouldContinue) {
                ...
                this.$input.trigger('memberPicker');
            ...
            MemberInput.prototype.showTips = function (value) {
                ...
                var highlightRegExp = new RegExp('^(' + newValue + ')', 'i')
                ...
                var tipsHtml = ret.map(function (tipsItem) {
                    var username = tipsItem.replace(/\([^\)]*\)/g, "")
                    return '<li><a href="javascript:;">' + tipsItem.replace(highlightRegExp, '<em class="oaui_memberinput_highlight">$1</em>') + '</a></li>'
            ...
            MemberInput.prototype.dataInit = function () {
                ...
                if (_this.$input.val() || initUserName) {
                    var existValue = _this.$input.val() || initUserName;
                    ...
                    _this.inputFinished(_this.$editor.find('input').get(), false)
                ...
                nameArr.forEach(function (name) {
                    ...
                    if (!allMemberList || !allMemberList.length || (options.allowMailGroup && groupMailRegExp.test(name))) {
                        ...
                        var inAllMember = allMemberList.some(function (allMemberItem, index) {
                            if (allMemberItem.toLowerCase() === fullName.toLowerCase()) {
                                targetIndex = index
                                return true
                            } else {
                                return false
                        ...
                        var inValue = _this.memberPickerValue.some(function (valueItem) {
                            return valueItem.toLowerCase() === fullName.toLowerCase()
                        })
            ...
            MemberInput.prototype.bindInputSize = function () {
                ...
                this.$editor.on('input', 'input', function (e) {
                  ...
                  if (options.maxMemberCount) {
                    var namePos = $(this).prevAll('.oaui_memberinput_member').length + $(this).nextAll('.oaui_memberinput_member').length

                    if (namePos > (options.maxMemberCount-1)) {
                      this.value = ''
                    } else {
                ...
                function bindTextInput() {
                    ...
                    if (num == 0) {
                        _this.$editor.on('input.textinput input', 'input', function (e) {
                            ...
                            if (_this.$editor.find('input').position().left + 200 > _this.$wrapper.width()) {
                                _this.$tips.css('right', 0)
                                _this.$tips.css('left', 'initial')
                            } else {
                                _this.$tips.css('left', _this.$editor.find('input').position().left)
                                _this.$tips.css('right', 'initial')
                            }
             ...
             */
            MemberInput.prototype.appendValue = function (value, shouldContinue) {
                this.setFocus({
                    x: this.$editor.width(),
                    y: this.$editor.height()
                })
                this.$editor.find('input').val(value)
                this.inputFinished(this.$editor.find('input').get(), shouldContinue)
            }
