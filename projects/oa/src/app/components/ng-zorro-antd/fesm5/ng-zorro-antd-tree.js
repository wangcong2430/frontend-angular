...
var NzTreeNodeComponent = /** @class */ (function () {
    ...
    Object.defineProperty(NzTreeNodeComponent.prototype, "displayStyle", {
         ...
         */
        function () {
            //这里只判断含有alltitle全标题的，若其它位置需要过滤功能需要添加alltitle字段
            if(typeof(this.nzTreeNode.origin.alltitle) != "undefined"){
                this.nzHideUnMatched = true;
                //isMatch不通过title来判断，而通过alltitle来判断
                if(this.nzTreeNode.origin.alltitle.indexOf(this.nzSearchValue) !== -1){
                    return '';
                }
                //原插件逻辑是有筛选条件时，收缩结构时会消失，这里加个数据过滤解决
                var origin_json = JSON.stringify(this.nzTreeNode.origin);
                var pattern = new RegExp("title\\\":\\\"[^\"]*"+this.nzSearchValue+"[^\"]*\\\"","g")
                if(pattern.test(origin_json) === true){
                    return '';
                }
            }
