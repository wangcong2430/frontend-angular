...
export class FormlyFieldCheckboxComponent extends FieldType implements OnChanges, OnInit {
    ...
    if (this.to.type === 'checkbox-group') {
      ...
      if (this.to['options']) {
        ...
        this.to['options'].forEach(item => {
              if (item['value'] && this.model[this.key] && this.model[this.key].some(val => val === item['value'])) {
                  item['checked'] = true;
                }else{
                  item['checked'] = false;
              }
              item['width'] = this.to['width'];
          });
      }
    }
  }
  ...
  checkall () {
    ...
    if (this.to.options instanceof  Array) {
      this.to.options = this.to.options.map(item => {
        return {
          ...item,
          checked: this.status
        };
      ...
      const value = this.to.options.filter(item => item.checked).map(item => item.value);
