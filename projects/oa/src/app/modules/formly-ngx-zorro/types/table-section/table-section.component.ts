...
export class TableSectionComponent extends FieldArrayType implements OnInit, OnDestroy {
            ...
            res['data'].map(item => {
              ...
              window.setTimeout(() => {
                ...
                for(let produce_breakdown of produce_breakdowns){
                  produce_breakdown.value = item['【明细数量】'+produce_breakdown.label]
                  if(item['【明细单位】'+produce_breakdown.label] != -1){
                    produce_breakdown.workload_unit_id = item['【明细单位】'+produce_breakdown.label]
                    produce_breakdown.unit_price = item['【明细单位价格】'+produce_breakdown.label]
                    produce_breakdown.workload_unit_name = item['【明细单位中文】'+produce_breakdown.label]
                  }
                }
  ...
  trackByFn(index, item) {
    ...
    customBigImgReq = (item: UploadXHRArgs) => {
      const id = this.message.loading('正在上传展示作品..', { nzDuration: 0 }).messageId;
      return this.uploadService.uploadBig(item, 1800, data => {
        this.message.remove(id);
        this.message.success('展示作品上传成功');
      });
    }
