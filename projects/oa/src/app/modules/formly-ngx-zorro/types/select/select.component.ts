...
export class FormlyFieldSelectComponent extends FieldType implements OnInit,  OnDestroy {
  ...
  ngOnInit() {
    ...
    if(this.to['onModelChange']){
      this.onModelChange = this.to['onModelChange'];
    }
    // 如果是异步，则搜索
    if (this.isAsyn) {
      // 当前是异步搜索
    ...
    this.formControl.valueChanges.pipe(distinctUntilChanged()).pipe(takeUntil(this.destroy$)).pipe(debounceTime(20)).subscribe(value => {
      this.description = '';
      if (Array.isArray(this.to['options']) && this.to['options'].length > 0) {
  ...
  onModelChange($event: any): void {
    if (this.to.onModelChange) {
      this.to.onModelChange($event, this.field)
    }
  }
