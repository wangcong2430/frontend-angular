<template>
  <a-breadcrumb>
    ...
    <a-breadcrumb-item>评分审批</a-breadcrumb-item>
  </a-breadcrumb>
  <div class="scoped-acceptance-approve-css mt-3 py-3 rounded-[4px] bg-white flex flex-col justify-start">
      ...
      <template #buttons>
        <a-space>
          <a-button :disabled="xTable && !(xTable.isAllCheckboxChecked() || xTable.isAllCheckboxIndeterminate())"
            type="primary" @click="submit_rows">
            <template #icon>
              <svg-icon name="submit" />
            </template>通过
          </a-button>
          <a-button type="danger"
            :disabled="xTable && !(xTable.isAllCheckboxChecked() || xTable.isAllCheckboxIndeterminate())"
            @click="handle_show_reject_form">
            <template #icon>
    ...
    <vxe-toolbar class="!px-[22px] !py-[24px] min-h-[70px]">
      <template #buttons>
        ...
        <span v-if="xTable?.getCheckboxRecords().length > 0" class="text-sm ml-3">已选中 {{
    ...
    <vxe-table ref="xTable" :expand-config="{ expandAll: true }" :show-header="false" row-class-name="tb-row"
      ...
      <vxe-column type="expand" :visible="false">
        <template #content="{ row }">
          <div class="px-[70px] w-full h-full py-3">
            <div class="grid xl:grid-cols-3 2xl:grid-cols-5 gap-1">
              <div v-for="option in row.score_options" class="font-semibold flex">
                <label>{{ option.score_option.name }}：</label>
                <span class="mr-2">
                  {{ option.score_option.score }} 分
                </span>
                <a-tooltip v-if="option.score_option.describe" :title="option.score_option.describe" class="pt-1">
                  <InfoCircleOutlined class="opacity-[0.4]" />
                </a-tooltip>
              </div>
              <div class="font-semibold flex">
                <label>评语：</label>
                <span class="w-full whitespace-normal break-all">
                  {{ row.annotation }}
                </span>
              </div>
    ...
    <suspense>
      <TransferForm v-if="show_transfer_form" :onClose="() => { show_transfer_form = false }" :onUpdate="fetch_data"
        node="11300" :rows="xTable.getCheckboxRecords()" />
    </suspense>
    <suspense>
      <RejectForm v-if="show_reject_form" :onClose="() => { show_reject_form = false }" :onUpdate="reject_rows" />
    </suspense>
  </div>
</template>

<script lang="ts">
import { defineComponent, reactive, ref, watch, computed, nextTick, inject } from 'vue'
import * as R from 'ramda'
import { InfoCircleOutlined } from '@ant-design/icons-vue';
import TransferForm from '@/components/perf_score/transfer_form.vue'
import RejectForm from '@/components/perf_score/acceptance_reject_form.vue'
...
import { get_acceptance_approve_score_list, post_approve_submit } from '@/api/perf/acceptance-approve'
...
export default defineComponent({
  ...
  async setup() {
    ...
    const fetch_data = async () => {
      ...
      const { list, pager: { page, itemCount: total, pageSize }, title } = await get_acceptance_approve_score_list(pager)
      data.tableData = list
      data.title = title
    ...
    const handle_show_detail_row = (row: any) => {
      ...
      //   }
      // }, '\*')
      if (!popup_in_parent(DemandDetail, { row, type: '11300' })) {
    ...
    const handle_show_transfer_form = () => {
      if (!popup_in_parent(TransferForm, { rows: xTable.value.getCheckboxRecords(), node: '11300', onUpdate: fetch_data })) {
        show_transfer_form.value = true
      }
    }

    const show_reject_form = ref(false)

    const handle_show_reject_form = () => {
      if (!popup_in_parent(RejectForm, { onUpdate: reject_rows })) {
        show_reject_form.value = true
      }
    }
    ...
    const submit_rows = async () => {
      ...
      try {
        const perf_score_detail_id = R.map(R.prop('id'), R.flatten(R.map(R.propOr([], 'score_options'), checked_rows)))
        const res = await post_approve_submit({ status: 0, perf_score_detail_id })
    ...
    const reject_rows = async (reason: string) => {
      const checked_rows = xTable.value?.getCheckboxRecords()
      if (R.length(checked_rows) === 0) {
        message.error('请先选择提交数据')
        return
      ...
      try {
        const perf_score_detail_id = R.map(R.prop('id'), R.flatten(R.map(R.propOr([], 'score_options'), checked_rows))).toString()
        const res = await post_approve_submit({ status: 3, perf_score_detail_id, reason })
        message.info(res.msg)
        fetch_data()
      } catch (e) {
        message.error(e.message)
