...
export class StyleComponent implements OnInit {
  ...
  getConfig() {
    this.loading = true;
    this.http.get('web/style/config').subscribe(result => {
      this.loading = false;
  ...
  getList(pagination?: null) {
    ...
    this.http.get('web/style/list', { params: params }).subscribe(result => {
  ...
  // 编辑框
  submitDel(item): void {
    this.http.post('web/style/del', { id: item['id'] }).subscribe(result => {
      if (result['code'] !== 0) {
  ...
  submitSaveForm(value): void {
    if (value['code'] === 0) {
      ...
      this.http.post('web/style/save', { params: value['value'] }).subscribe(result => {
