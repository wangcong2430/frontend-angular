...
export class ExchangeRateComponent implements OnInit {
  ...
  getConfig() {
    this.loading = true;
    this.http.get('web/exchange-rate/config').subscribe(result => {
      this.loading          = false;
  ...
  getList(pagination?: null) {
    ...
    this.http.get('web/exchange-rate/list', { params: params }).subscribe(result => {
      if (result['code'] !== 0) {
...
// 点击事件
  ...
  openModal(item = {}) {
    ...
    this.http.get('web/exchange-rate/save-config', { params: params }).subscribe(result => {
      if (result['code'] !== 0) {
        this.message.error(result['msg']);
        return false;
      }
  ...
  submitSaveForm(value): void {
    if (value['code'] === 0) {
      ...
      this.http.post('web/exchange-rate/save', { ...value['value'] }).subscribe(result => {
  ...
  openLogModal() {
    ...
    this.changeList = [];
    this.http.get('web/exchange-rate/change-list').subscribe(result => {
