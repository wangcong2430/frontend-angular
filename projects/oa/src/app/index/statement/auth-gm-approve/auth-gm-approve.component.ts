...
@Component({
  templateUrl: './auth-gm-approve.component.html',
})

export class AuthGmApproveComponent implements OnInit {
  // loading
  loading = true;
  listLoading = false;
  // 配置项
  ...
  getList(pagination?: null) {
    ...
    this.http.get('web/statement/auth-gm-approve-list', {
      params: {
        group_by: '1',
        ...this.pagination
      }
  ...
  approvalSubmit(optionType) {
    const thing_id = [];
    this.list.forEach((story, statementIndex) => {
      if (story.checked) {
    ...
    this.http.post('/web/statement/gm-approve-submit', {
      thing_id: thing_id,
