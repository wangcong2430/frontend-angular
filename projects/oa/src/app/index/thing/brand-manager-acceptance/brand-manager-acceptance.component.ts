...
@Component({
  templateUrl: './brand-manager-acceptance.component.html',
})

export class BrandManagerAcceptanceComponent implements OnInit {
  loading = true;
  listLoading = false;
  // 配置项
  columns = [];
...
// 筛选
  ...
  getList(pagination?: null) {
    ...
    this.http.get('web/acceptance/brand-manager-acceptance-list', {
      params: params
    }).subscribe(result => {
  ...
  acceptanceSubmit(optionType, hint = true) {
    ...
    this.http.post('/web/acceptance/brand-manager-acceptance-' + optionType,
      {
        thing_id: thing_id,
