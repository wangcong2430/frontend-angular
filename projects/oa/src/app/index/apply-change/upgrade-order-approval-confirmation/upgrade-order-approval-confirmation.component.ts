...
@Component({
  templateUrl: './upgrade-order-approval-confirmation.component.html',
})

export class UpgradeOrderApprovalConfirmationComponent implements OnInit {
  constructor(
  ...
  getConfig() {
    ...
    this.http.get('web/apply-change/order-change-config', { params }).subscribe(result => {
      this.loading = false;
  ...
  getList(pagination?: null) {
    ...
    this.http.get('web/apply-change/upgrade-order-approval-confirmation-list', { params: params}).subscribe(result => {
      this.listLoading            = false;
      if (result['code'] === 0 && typeof(result['list']) != 'undefined') {
  ...
  confirmationSubmit(optionType) {
    ...
    this.http.post('/web/apply-change/upgrade-order-approval-confirmation-' + optionType, {
        thing_id: thing_id,
        reason: this.reason
      }
    ).subscribe(result => {
