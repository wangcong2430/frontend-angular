...
export class SystemConfigComponent implements OnInit {
  ...
  getConfig() {
    this.loading = true;
    this.http.get('web/maintenance/system-config').subscribe(result => {
      this.loading          = false;
  ...
  getList(pagination?: null) {
    ...
    this.http.get('web/maintenance/system-config-list', { params: params }).subscribe(result => {
  ...
  submitSaveForm(value): void {
    if (value['code'] === 0) {
      ...
      this.http.post('web/maintenance/system-config-save', { params: value['value'] }).subscribe(result => {
